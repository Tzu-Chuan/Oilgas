...
public class OilCommitteeSuggestion_DB
{
    ...
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"update 石油_自評表委員意見log 
...
set 答案=@答案, 檢視文件=@檢視文件, 修改者=@修改者, 修改日期=@修改日期  
where 委員guid=@委員guid and 業者guid=@業者guid and 題目guid=@題目guid and 年度=@年度 
and 答案=@答案 and 委員意見=@委員意見");
        SqlCommand oCmd = oConn.CreateCommand();
        oCmd.CommandText = sb.ToString();
    ...
    public void UpdateSuggestion(SqlConnection oConn, SqlTransaction oTran)
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(@"update 石油_自評表委員意見log 
set 檢視文件=@檢視文件, 修改者=@修改者, 修改日期=@修改日期  
where 委員guid=@委員guid and 業者guid=@業者guid and 題目guid=@題目guid and 年度=@年度 
and 答案=@答案 and 委員意見=@委員意見");
        SqlCommand oCmd = oConn.CreateCommand();
        oCmd.CommandText = sb.ToString();
...
select *,
Row_Number() over (PARTITION BY 委員guid order by 修改日期 desc) SN
from 石油_自評表委員意見log) R
WHERE 委員guid=@委員guid and 年度=@年度 and 業者guid=@業者guid and 題目guid=@題目guid and 資料狀態=@資料狀態 and R.SN='1' ");
